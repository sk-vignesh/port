                  ...
                  <tbody>
                    {recentAcctTxn.map(tx => (
                      <tr key={tx.id}>
                        ...
                        <td className={`table-right text-sm ${['DEPOSIT','DIVIDENDS','INTEREST','SELL'].includes(tx.type) ? 'amount-positive' : 'amount-negative'}`}>{formatAmount(tx.amount, tx.currency_code)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
