...
const TX_TYPES = [
  ...
  { value: 'DIVIDEND',         label: 'Dividend',     color: '#3b82f6' },
  { value: 'BONUS',            label: 'Bonus Shares', color: '#8b5cf6' },
  { value: 'SPLIT',            label: 'Stock Split',  color: '#f59e0b' },
  { value: 'DELIVERY_INBOUND', label: 'Delivery In',  color: '#06b6d4' },
  { value: 'TRANSFER_IN',      label: 'Transfer In',  color: '#64748b' },
  { value: 'TRANSFER_OUT',     label: 'Transfer Out', color: '#475569' },
]
...
export default function EquityTradeForm({ portfolioId }: { portfolioId: string }) {
  ...
  useEffect(() => {
    ...
    setResolving(true)
    supabase.from('securities').select('id, name, ticker_symbol, currency_code').eq('id', urlSecurityId).maybeSingle()
      .then(({ data }) => {
        if (data) setSelected({ id: data.id, name: data.name, ticker: data.ticker_symbol ?? '', currency: data.currency_code })
        setResolving(false)
      })
  ...
  // For dividend: amount = total dividend received; shares = 0
  ...
  const handleSelectStock = async (stock: IndianStock) => {
    ...
    if (existing) {
      ...
      const { data: created, error: cErr } = await supabase.from('securities').insert({ user_id: user!.id, name: stock.name, ticker_symbol: stock.symbol, currency_code: 'INR', feed: 'YAHOO' }).select('id').single()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isDividend && (!price || +price <= 0)) { setError('Enter total dividend amount'); return }
    if (!isDividend && !isSplit && (!shares || +shares <= 0 || !price || +price <= 0)) { setError('Enter shares and price'); return }
    setLoading(true); setError(null)

    let txShares = Math.round(+shares * 100_000_000)
    let txAmount = Math.round(+shares * +price * 100)
    if (isDividend) { txShares = 0; txAmount = Math.round(+price * 100) }
    if (isSplit)    { txAmount = 0 }
    ...
    const { error: err } = await supabase.from('portfolio_transactions').insert({
      ...
      shares: txShares, amount: txAmount, currency_code: selected.currency, date, note: note || null,
      unit_type: 'SHARES',
    })
    ...
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="card">
        <div style={{ padding: '18px 20px' }}>
          <div style={{ fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--color-text-muted)', marginBottom: 10 }}>Stock / Security</div>
          {!selected
            ? resolving
              ? <div style={{ padding: '12px 0', color: 'var(--color-text-muted)', fontSize: '0.875rem' }}>Resolving…</div>
              : <SecuritySearchInput onSelect={handleSelectStock} />
            : <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: '1.05rem', marginBottom: 3 }}>{selected.name}</div>
                  <div style={{ display: 'flex', gap: 8, fontSize: '0.78rem' }}>
                    <code style={{ color: 'var(--color-accent-light)', background: 'var(--color-accent-glow)', padding: '1px 7px', borderRadius: 4 }}>{selected.ticker}</code>
                    {selected.sector && <span style={{ color: 'var(--color-text-muted)' }}>{selected.sector}</span>}
      ...
      {selected && (<>
        ...
        {/* Quantity & Price — context-sensitive labels */}
        <div className="card"><div style={{ padding: '18px 20px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: isDividend ? '1fr' : '1fr 1fr', gap: 16 }}>
            {!isDividend && (
              <div>
                <label style={{ display: 'block', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--color-text-muted)', marginBottom: 8 }}>
                  {isSplit ? 'New Share Count' : 'Shares *'}
                </label>
                <input type="number" className="form-input" step="1" min="1" placeholder="0" value={shares} onChange={e => setShares(e.target.value)} style={{ fontSize: '1.1rem', fontWeight: 600, textAlign: 'right' }} required={!isDividend} />
              </div>
            )}
            <div>
              <label style={{ display: 'block', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--color-text-muted)', marginBottom: 8 }}>
                {isDividend ? `Dividend Amount (${selected.currency}) *` : isSplit ? 'Pre-split Price (optional)' : `Price / Share (${selected.currency}) *`}
              </label>
              <input type="number" className="form-input" step="0.01" min="0" placeholder="0.00" value={price} onChange={e => setPrice(e.target.value)} style={{ fontSize: '1.1rem', fontWeight: 600, textAlign: 'right' }} required={isDividend} />
            </div>
          </div>
          {totalAmount !== null && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 16px', borderRadius: 'var(--radius-md)', background: `${activeType.color}12`, border: `1px solid ${activeType.color}40`, marginTop: 16 }}>
              <span style={{ fontSize: '0.82rem', color: 'var(--color-text-muted)', fontWeight: 500 }}>{fmtNum(+shares)} shares × ₹{fmtNum(+price)}</span>
              <span style={{ fontSize: '1.3rem', fontWeight: 800, color: activeType.color }}>{fmt(totalAmount, selected.currency)}</span>
            </div>
          )}
        </div></div>
        ...
        <div className="card"><div style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--color-text-muted)', marginBottom: 8 }}>Note <span style={{ fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(optional)</span></label>
            <textarea className="form-input" rows={2} placeholder="e.g. SIP, Q3 dividend…" value={note} onChange={e => setNote(e.target.value)} />
          </div>
        </div></div>
      </>)}
      ...
      <div style={{ display: 'flex', gap: 12, paddingBottom: 32 }}>
        <button type="submit" disabled={loading || !selected}
          style={{ flex: 1, padding: '14px 24px', borderRadius: 'var(--radius-md)', fontWeight: 700, fontSize: '1rem', border: 'none', cursor: selected ? 'pointer' : 'not-allowed', background: selected ? `linear-gradient(135deg, ${activeType.color}, ${activeType.color}cc)` : 'var(--color-bg-input)', color: selected ? '#fff' : 'var(--color-text-muted)', boxShadow: selected ? `0 4px 20px ${activeType.color}44` : 'none', transition: 'all 0.2s' }}>
